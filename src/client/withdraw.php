...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($amount <= 0) {
        ...
        try {
            ...
            $stmt2 = $db->prepare("INSERT INTO transactions (account_id, type, amount, description) VALUES (:aid, 'withdraw', :amt, :desc)");
            $stmt2->execute(['aid' => $accountId, 'amt' => $amount, 'desc' => $description ?: 'Withdrawal']);

            $db->commit();
            $success = 'Withdrawal of $' . number_format($amount, 2) . ' completed successfully.';
    ...
    <div class="container">
        ...
        <div class="card">
            <div style="margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid #eee;">
                ...
                &middot;
                <strong>Available Balance:</strong> $<?= number_format($account['balance'], 2) ?>
            </div>
